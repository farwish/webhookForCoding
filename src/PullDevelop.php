...
/**
 * Develop Project Webhook.(for coding.net)
 *
 * 约定:
 *
 *  如果项目目录是 /home/wwwroot/Discuz 
 *  那么你应该建一个 Discuz.develop.php , 这里约定这个文件名前缀就是项目目录名.
 *      ( 主要为了一目了然, 定义好一个项目的WebHook文件后, 其余项目与之区别仅在于文件名. )
 *      ( 后缀并不要求, 这里是为了与 Discuz.master.php 区分. )
 *  编辑 Discuz.develop.php 加入如下code部分.
 *
 * <code>
 *  include('./src/autoload.php');
 *  (new \WebHook\PullDevelop('/home/wwwroot', 9981, 'log.txt')); // 对应类中三个属性.
...
class PullDevelop implements PullInterface
{
    /**
     * 项目所在上级目录.
     */
    ...
    public function doit()
    {
        ...
        if (
            ...
            ||  
                ...
                && ($hook_array['ref'] == 'refs/heads/develop')
                && ($hook_array['event'] == 'push')
                )
            )   
            ...
            if ( isset($hook_array['merge_request']) ) { 
            ...
            } else if ( isset($hook_array['event']) ) { 
                $core = 'Repo: ' . 
                    $hook_array['repository']['name'] . 
                    ', ' . $hook_array['event'] . ':(' . $hook_array['ref'] . ')';
            } else {
            }   
